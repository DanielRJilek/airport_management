...
BEGIN
    ...
    SET @val_match = match_value;
    SET @sql = CONCAT(
        'DELETE FROM Airport WHERE ', match_attr, ' = ?'
    );

    PREPARE stmt FROM @sql;
    EXECUTE stmt USING @val_match;
    DEALLOCATE PREPARE stmt;
...
main_block: BEGIN
    ...
    IF match_attr NOT IN ('airline_name', 'country', 'airport_name', 'airline_id') THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'Invalid delete attribute. Allowed: airline_name, country, airport_name, airline_id';
    END IF;

    IF match_value IS NULL THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'Delete value cannot be NULL';
    END IF;
    ...
    SET @sql = CONCAT(
        'DELETE FROM Airline WHERE ', match_attr, ' = ?'
    );
...
main_block: BEGIN
    ...
    IF match_attr NOT IN ('terminal_name','airport_name','num_gates','terminal_id') THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'Invalid delete attribute';
    END IF;
    ...
    SET @sql = CONCAT(
        'DELETE FROM Terminal WHERE ', match_attr, ' = ?'
    );
...
main_block: BEGIN
    ...
    SET @m = match_value;
    SET @sql = CONCAT('DELETE FROM Gate WHERE ', match_attr, ' = ?');
...
DROP PROCEDURE IF EXISTS create_Aircraft;
DELIMITER $$
...
main_block: BEGIN
    ...
    SET @sql = CONCAT(
        'DELETE FROM Aircraft WHERE ', match_attr, ' = ?'
    );
...
main_block: BEGIN
    ...
    IF match_attr NOT IN (
        'flight_id','flight_number','flight_type','status',
        'departure_time','arrival_time','aircraft_id','gate_id',
        'airline_name','arrival_airport_name','destination_airport_name'
    ) THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'Invalid delete attribute';
    END IF;
    ...
    IF (SELECT COUNT(*) FROM Flight WHERE
        (match_attr='flight_id' AND flight_id = match_val) OR
        (match_attr='flight_number' AND flight_number = match_val) OR
        (match_attr='airline_name' AND airline_name = match_val) OR
        (match_attr='aircraft_id' AND aircraft_id = match_val) OR
        (match_attr='gate_id' AND gate_id = match_val)
    ) = 0 THEN
        SIGNAL SQLSTATE '45000'
    ...
    SET @sql = CONCAT('DELETE FROM Flight WHERE ', match_attr, ' = ?');
...
CREATE PROCEDURE delete_Seat(
    IN match_attr VARCHAR(50),
    IN match_val  VARCHAR(100)
)
BEGIN
    IF match_attr NOT IN ('seat_id','seat_number','class','status','flight_id') THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'Invalid delete attribute';
    END IF;
    ...
    SET @exists_sql = CONCAT(
        'SELECT COUNT(*) INTO @exists FROM Seat WHERE ', 
        match_attr, ' = ?'
    );

    PREPARE stmt1 FROM @exists_sql;
    EXECUTE stmt1 USING @v;
    DEALLOCATE PREPARE stmt1;

    IF @exists = 0 THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'No matching seat to delete';
    END IF;
